...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
                 ...
                 take_best_prototypes=True, t=0.8):

        super(PPNet, self).__init__()
        self.take_best_prototypes = take_best_prototypes
...
#        prototype_activations = self.binarizer(prototype_activations) - self.t
#        if self.thresholds is not None:
#            prototype_activations = torch.ge(prototype_activations, self.thresholds).float()
    ...
    def forward_bin(self, x):
        ...
        min_distances = min_distances.view(-1, self.num_prototypes)
        prototype_activations = self.distance_2_similarity(min_distances)
#        fifth = torch.kthvalue(prototype_activations, k=1990, dim=1)[0]
#        prototype_activations = torch.where(torch.lt(prototype_activations, fifth.view(-1, 1)), torch.tensor([0.]).cuda(), prototype_activations)
#        prototype_activations = torch.where(torch.lt(prototype_activations, fifth.view(-1, 1)), torch.tensor([0.]).cuda(), torch.tensor([1.]).cuda())
        prototype_activations = self.binarizer(prototype_activations) - self.t
        if self.thresholds is not None:
            prototype_activations = torch.ge(prototype_activations, self.thresholds).float()
        logits = self.last_layer(prototype_activations)
    ...
    def make_thresholds(self, thresholds):
...
def construct_PPNet(base_architecture, pretrained=True, img_size=224,
    ...
    return PPNet(features=features,
                 ...
                 take_best_prototypes=take_best_prototypes)
